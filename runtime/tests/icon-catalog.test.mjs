...
import { iconCatalog, iconRawValues, iconCatalogVersion, resolveIcon } from "../icon-catalog.mjs";
...
test("number, progress, filled and disabled variants retain distinct artwork", () => {
  const artwork = name => digest(fs.readFileSync(resolveIcon(name)));
...
test("launcher and bundled extension icon references all resolve", () => {
  ...
  assert.equal(new Set(diagonals.map(name => digest(fs.readFileSync(resolveIcon(name))))).size, 4);
});
