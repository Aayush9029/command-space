...
import {lookupCredential, clearCredential, type CredentialAccess} from "../oauth.ts";

const run = promisify(execFile);
const runtime = fileURLToPath(new URL("../", import.meta.url));

function credentials(attributes: string[], entries: [string, string][], failStore = false) {
  const secrets = new Map(entries.map(([application, value]) => [JSON.stringify(["application", application, ...attributes]), value]));
  const access: CredentialAccess = async ([operation, ...arguments_], value) => {
...
test("browser migration reconnects existing registrations and preserves other extensions", async t => {
  ...
  for (const file of ["install-browser.ts", "browser-config.ts", "build-browser.ts", "adapter-utils.ts", "browser-extension"]) {
  ...
  const installer = path.join(target, "install-browser.ts");
  await run(process.execPath, [installer], {env});
  for (const directory of [chromium, firefox]) {
    for (const name of [legacyName, "com.superspace.bridge"]) {
      const manifest = JSON.parse(await fs.readFile(path.join(directory, `${name}.json`), "utf8"));
  ...
  const manifest = JSON.parse(await fs.readFile(path.join(target, "browser-extension/manifest.json"), "utf8"));
  assert.equal(manifest.key, JSON.parse(await fs.readFile(path.join(runtime, "browser-extension/manifest.json"), "utf8")).key);
