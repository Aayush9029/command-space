...
On the development ARM64 VM, the optimized launcher measured 59.68 ms from service restart to a mapped window and 14.33 ms median / 23.00 ms p95 for twenty warm opens. After settling, the hidden service used 58.06 MiB and 0.117% of one CPU core, including the Days Until Christmas background worker. These measurements use IPC and Hyprland window-state polling; they do not measure display frame latency.
