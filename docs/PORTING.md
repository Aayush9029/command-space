...
The GitHub workflows target Linux ARM64 and x86-64. Both hosted CI architectures passed at commit `c06d78e` in [run 34421177603](https://github.com/Aayush9029/command-space/actions/runs/34421177603). That run predates the latest UI, native workflow, and runtime changes. ARM64 archives have been built and verified locally; the final source revision still needs hosted CI and an x86-64 package. No release is published yet.

## Remaining work

The port is in progress. Package metadata, the AUR build-script view, package removal cancellation, and the monospace branding editor have passed native interaction checks. The new administrative forms have host and script integration coverage; live DNS, SSH, Windows VM, and encrypted-drive changes were not executed as part of those tests.

Final validation includes cold-start package loading and the remaining native interaction checks, refreshed startup and idle-resource measurements, hosted CI for that revision, both release architectures, and installation, update, and reboot checks against the final artifacts. Release publication follows those checks. Final native and performance checks use the optimized release profile; the debug profile remains available for iteration.

The Chromium and Firefox bridges have passed their native integration checks. Additional extension APIs must retain isolated and desktop coverage as they land. Compatibility with an arbitrary extension depends on its APIs, external programs, and service requirements; the verified examples do not establish universal Raycast extension support.
