...
function post(value){queue = queue.then(()=>fetch('/input?token='+token,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(value)})).then(r=>{if(!r.ok)throw Error(r.status);setTimeout(refresh,600)}).catch(e=>document.querySelector('#status').textContent=e.message);return queue}
let frameURL;
...
async function refresh(){
  ...
  refreshing=true;
  try {
    const response = await fetch('/frame?token='+token+'&t='+Date.now());
    if(!response.ok)throw Error('Frame delayed');
    const next = URL.createObjectURL(await response.blob());
    const previous = frameURL; frameURL=next; const frame=new Image(); frame.src=next; await frame.decode(); screen.width=frame.naturalWidth; screen.height=frame.naturalHeight; screen.getContext('2d').drawImage(frame,0,0);
    if(previous)URL.revokeObjectURL(previous);
    document.querySelector('#status').textContent='Live';
  }catch(error){document.querySelector('#status').textContent='Frame delayed'}
  refreshing=false;
}
