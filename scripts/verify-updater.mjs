...
try {
  ...
  const valid = await fs.readFile(binary), validHost = await fs.readFile(path.join(target,"runtime/host.mjs"));
  ...
  assert.deepEqual(await fs.readFile(path.join(target,"runtime/host.mjs")),validHost);
  assert.deepEqual(await fs.readFile(config),originalConfig);
