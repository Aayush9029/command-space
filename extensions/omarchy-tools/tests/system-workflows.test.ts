...
import { dnsServers, configureDNS, sshKeys, windowsConfiguration, encryptedDrives, drivePassword, launchTerminalOperation, consumeOperation, parseOperation } from "../src/system-workflows.ts";
import { performOperation } from "../assets/native-operation.ts";
import { run, type Execute } from "../src/workflows.ts";
import { readBranding, writeBranding, resetBranding, type BrandingTarget } from "../src/branding.ts";
...
test("custom DNS validates addresses and supplies data to original stdin", async () => {
  ...
  const operations: Parameters<Execute>[] = [];
  await configureDNS({ servers: "192.168.1.1 1.1.1.1" }, { execute: async (...args: Parameters<Execute>) => { operations.push(args); return ""; } });
...
test("SSH keys are verified before any setup and GitHub imports stay bounded", async t => {
  ...
  await assert.rejects(sshKeys({ source: "github", username: "fixture" }, { fetcher: async () => new Response(null, { status: 204 }) }), /did not return a key list/);
});
...
test("encrypted drive listings reject malformed roots and tolerate unrelated device data", () => {
  for (const value of [null, [], {}, { blockdevices: null }, { blockdevices: {} }]) {
    assert.throws(() => encryptedDrives(JSON.stringify(value)), /Invalid encrypted drive listing/);
  }
  assert.throws(() => encryptedDrives("{"), SyntaxError);
  const listing = { blockdevices: [
    null,
    { name: "/dev/vda", children: [false, { path: "/dev/vda2", fstype: "crypto_LUKS", size: "100G", label: "System" }] },
    { name: "/dev/vdb", fstype: "crypto_LUKS", size: 42, label: {}, children: {} },
    { path: "/tmp/not-device", fstype: "crypto_LUKS" },
    { path: 42, fstype: "crypto_LUKS" },
    { path: "/dev/vdc", fstype: "ext4" },
    { path: "/dev/vdd", fstype: "crypto_LUKS", model: "External" },
  ] };
  assert.deepEqual(encryptedDrives(JSON.stringify(listing)), [
    { path: "/dev/vda2", title: "/dev/vda2 · 100G · System" },
    { path: "/dev/vdb", title: "/dev/vdb" },
    { path: "/dev/vdd", title: "/dev/vdd · External" },
  ]);
  const depth = 10000;
  const nested = `{"blockdevices":[${'{"children":['.repeat(depth)}{"path":"/dev/deep","fstype":"crypto_LUKS"}${']}'.repeat(depth)}]}`;
  assert.deepEqual(encryptedDrives(nested), [{ path: "/dev/deep", title: "/dev/deep" }]);
});

test("authentication operation payloads validate their shape before dispatch", () => {
  const invalid: unknown[] = [
    null, [], {}, { operation: "sshd" }, { operation: "sshd", values: [] },
    { operation: "unknown", values: {} }, { operation: "sshd", values: { keys: "ssh-ed25519 key" } },
    { operation: "sshd", values: { keys: [42] } },
    { operation: "drive-password", values: { drive: "/dev/vda2", password: null } },
    { operation: "drive-password", values: { drive: 42, password: "secret" } },
    { operation: "windows", values: { password: "secret", username: "bad\nname" } },
  ];
  for (const value of invalid) assert.throws(() => parseOperation(value));
  const ssh = { operation: "sshd", values: { keys: ["ssh-ed25519 key"] } };
  const drive = { operation: "drive-password", values: { drive: "/dev/vda2", password: "secret" } };
  assert.deepEqual(parseOperation(ssh), ssh);
  assert.deepEqual(parseOperation(drive), drive);
});

test("authentication handoff uses private one-use data and never places secrets in argv", async t => {
  const invocations: Parameters<Execute>[] = [];
  const password = "private value with $(literal)";
  const file = await launchTerminalOperation("drive-password", { drive: "/dev/vda2", password }, { assetsPath, execute: async (...args: Parameters<Execute>) => { invocations.push(args); return ""; } });
...
test("drive executor retains cryptsetup authentication on the terminal and sends only new key over stdin", async () => {
  ...
  await performOperation({ operation: "drive-password", values: { drive: "/dev/vda2", password } }, {
    execute: async (...args: Parameters<Execute>) => { invocations.push(args); }, inspect: async () => devices, verifyBlockDevice: async () => true,
  });
  const invocation = invocations[0];
  assert.ok(invocation);
...
test("Windows executor frames native settings on stdin without exposing its password", async () => {
  ...
  await performOperation({ operation: "windows", values: config }, { execute: async (...args: Parameters<Execute>) => { invocations.push(args); } });
  const invocation = invocations[0];
  assert.ok(invocation);
...
test("branding edits and resets use installed defaults without starting a legacy preview", async t => {
  ...
  for (const target of ["about", "screensaver"] as const) {
  ...
  await assert.rejects(writeBranding("../escape" as BrandingTarget, "text", options), /Unknown branding/);
