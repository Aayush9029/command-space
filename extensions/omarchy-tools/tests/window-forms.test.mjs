...
import { moveBounds, resizeBounds } from "../src/window-bounds.mjs";
...
test("window dimensions accept exact pixel integers and reject lossy or invalid input", () => {
  ...
  for (const value of ["", "12.5", "1e3", "0x20", "NaN", "Infinity", "2147483648", "-2147483649"]) {
    assert.throws(() => moveBounds({ x: value, y: "0" }), error => error.field === "x");
  }
  for (const value of ["", "0", "-1", "1.5", "2147483648"]) {
    assert.throws(() => resizeBounds({ width: "800", height: value }), error => error.field === "height");
  }
});
...
for (const command of ["move-window", "resize-window"]) test(`${command} submits only the requested geometry to the captured window`, async t => {
  ...
  const action = nodes(rendered.tree).find(node => node.type === "Action.SubmitForm");
  host.send({ type: "event", callback: action.props.onAction.$callback, args: [moving ? { x: "-40", y: "60" } : { width: "640", height: "480" }] });
...
test("invalid window input displays a field error without dispatching", async t => {
  ...
  host.send({ type: "event", callback: nodes(rendered.tree).find(node => node.type === "Action.SubmitForm").props.onAction.$callback, args: [{ width: "640.5", height: "480" }] });
