...
import { webAppArguments, tuiArguments, reminderArguments, parseReminders, selectedPaths, transcodeArguments, brandingArguments, installWallpapers, installTheme, themeName, addShellPlugin, retroCores, retroGameArguments, run, type Execute } from "../src/workflows.ts";
...
test("reminder listings preserve values and reject malformed entries before rendering", () => {
  const reminder = { unit: "reminder-fixture.timer", label: "Check Café ☕", atTime: "Thu 2026-09-10 12:00:00 EDT", remaining: "5min" };
  assert.deepEqual(parseReminders(JSON.stringify({ reminders: [{ ...reminder, extra: "ignored" }] })), [reminder]);
  assert.deepEqual(parseReminders('{"reminders":[]}'), []);
  for (const value of [null, [], {}, { reminders: null }, { reminders: {} }]) {
    assert.throws(() => parseReminders(JSON.stringify(value)), /Invalid reminder listing/);
  }
  for (const entry of [null, [], {}, { ...reminder, unit: 42 }, { ...reminder, label: null }, { ...reminder, atTime: [] }, { ...reminder, remaining: false }]) {
    assert.throws(() => parseReminders(JSON.stringify({ reminders: [entry] })), /Invalid reminder entry/);
  }
  assert.throws(() => parseReminders("{"), SyntaxError);
});
...
test("wallpapers preserve existing images and only apply when requested", async t => {
  ...
  const execute: Execute = async (command, args) => { calls.push([command, args]); return "image/png"; };
...
test("theme installation protects installed themes and passes complete repository arguments", async t => {
  const home = await temporary(t);
  const calls: Parameters<Execute>[] = [];
  const execute: Execute = async (...args) => { calls.push(args); return ""; };
...
test("closing the extension worker terminates its operation process group", { skip: process.platform === "win32" }, async t => {
  ...
  const module = fileURLToPath(new URL("../src/workflows.ts", import.meta.url));
