#!/usr/bin/env bash
set -euo pipefail
IFS= read -r -d '' super_space_ram
IFS= read -r -d '' super_space_cores
IFS= read -r -d '' super_space_disk
IFS= read -r -d '' super_space_username
IFS= read -r -d '' super_space_password
